...
export default function Navbar() {
 ...
 {user && (
            <>
              <Link href="/write" className="hover:text-gray-300 flex items-center text-md md:text-lg gap-1 md:gap-2">
                <Pencil className="w-4 h-4 md:w-5 md:h-6" /> Write
              </Link>
              <Link href="/mystories" className="hover:text-gray-300 flex items-center text-md md:text-xl gap-1 md:gap-2">
    ...
    {user ? (
      <div className="relative group">
        <span className="cursor-pointer text-md flex md:gap-1 md:text-xl"><User className="w-5 h-5 md:w-6 md:h-6" /> {user.name}</span>
